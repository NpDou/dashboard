<template>
  <div>
    <a-alert v-if="isWinstack" class="my-2" :message="$t('cloudenv.text_252')" banner />
    <a-form :form="form.fc" v-bind="formLayout">
      ...
      <a-form-item :label="$t('cloudenv.text_253')">
        ...
        <span slot="extra">
          {{isWinstack ? $t('common_569') : $t('common_570')}}
        </span>
      </a-form-item>
      ...
      <a-form-item :label="$t('cloudenv.text_254')"  v-if="isWinstack">
        <a-input v-decorator="decorators.project_name" :placeholder="$t('cloudenv.text_255')" />
      </a-form-item>
      <a-form-item label="Domain Name"  v-if="isWinstack">
...
export default {
  ...
  computed: {
    isWinstack () {
      return this.provider.toLowerCase() === 'winstack'
    },
    urlPlaceholder () {
      if (this.isWinstack) {
    ...
    getDecorators (initKeySecretFields) {
      ...
      const decorators = {
        ...
        auto_create_project: [
          'auto_create_project',
          {
            initialValue: this.provider.toLowerCase() === 'winstack',
